        ...
        if (yesterday_sale == 0 || today_sale == 0) {
            for (var i in item) {
                for (var j in item[i]) {
                    ...
                    var after_refill = item[i][j].perdiction_value + item[i][j].stock_value

                    if (after_refill > item[i][j].max_capacity) {
                        var new_perdiction = item[i][j].max_capacity - item[i][j].stock_value
                        item[i][j]['perdiction_value'] = new_perdiction
                    }
                }
            }
        } else {
            ...
            for (var i in item) {
                for (var j in item[i]) {
                    var today_perdiction = item[i][j].base_number

                    if (typeof (yesterday_useage[item[i][j]._id]) != 'undefined') {
                        today_perdiction = yesterday_useage[item[i][j]._id].item_useage * ratio
